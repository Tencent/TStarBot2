...
class CorruptorMgr(MicroBase):
    ...
    def parasitic_bomb_attack_target(u, target):
        action = sc_pb.Action()
        action.action_raw.unit_command.ability_id = ABILITY_ID.EFFECT_PARASITICBOMB.value
    ...
    def act(self, u, pos, mode):
        ...
        if len(air_targets) > 0:
            ...
            if self.cal_square_dist(u, closest_target) > self.corruptor_range:
                ...
                if len(self_ground_units) == 0:
                    ...
                    return action
                self_most_dangerous_ground_unit = self.find_closest_units_in_battle(self_ground_units, closest_target)
