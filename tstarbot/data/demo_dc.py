...
class ZergLxHanDcMgr(PoolBase):
    """ full game with Simple64 by producing roaches + hydralisk (for testing combat module) """
    def __init__(self):
    ...
    def collect_drones(self, units):
        drones = []
        for u in units:
            if u.unit_type == tp.UNIT_TYPEID.ZERG_DRONE.value and u.int_attr.owner == 1:
                drones.append(u)
        self.drones = drones
    ...
    def collect_hatcheries(self, units):
        hatcheries = []
        for u in units:
            if u.unit_type == tp.UNIT_TYPEID.ZERG_HATCHERY.value and u.int_attr.owner == 1:
                hatcheries.append(u)
    ...
    def collect_larvas(self, units):
        larvas = []
        for u in units:
            if u.unit_type == tp.UNIT_TYPEID.ZERG_LARVA.value and u.int_attr.owner == 1:
                larvas.append(u)
        self.larvas = larvas
    ...
    def collect_queen(self, units):
        queen = []
        for u in units:
            if u.unit_type == tp.UNIT_TYPEID.ZERG_QUEEN.value and u.int_attr.owner == 1:
                queen.append(u)
        self.queen = queen
    ...
    def collect_spawningpool(self, units):
        spawningpool = []
        for u in units:
            if u.unit_type == tp.UNIT_TYPEID.ZERG_SPAWNINGPOOL.value and u.int_attr.owner == 1:
    ...
    def collect_extractor(self, units):
        extractors = []
        for u in units:
            if u.unit_type == tp.UNIT_TYPEID.ZERG_EXTRACTOR.value and u.int_attr.owner == 1:
                extractors.append(u)
    ...
    def collect_roachwarren(self, units):
        roachwarren = []
        for u in units:
            if u.unit_type == tp.UNIT_TYPEID.ZERG_ROACHWARREN.value and u.int_attr.owner == 1:
    ...
    def collect_roaches(self, units):
        roaches = []
        for u in units:
            if u.unit_type == tp.UNIT_TYPEID.ZERG_ROACH.value and u.int_attr.owner == 1:
                roaches.append(u)
        self.roaches = roaches
    ...
    def collect_hydraliskden(self, units):
        hydraliskden = []
        for u in units:
            if u.unit_type == tp.UNIT_TYPEID.ZERG_HYDRALISKDEN.value and u.int_attr.owner == 1:
    ...
    def collect_hydralisk(self, units):
        hydralisk = []
        for u in units:
            if u.unit_type == tp.UNIT_TYPEID.ZERG_HYDRALISK.value and u.int_attr.owner == 1:
                hydralisk.append(u)
